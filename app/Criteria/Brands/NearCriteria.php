...
class NearCriteria implements CriteriaInterface
{
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        if ($this->request->has(['longitude', 'latitude'])) {
            ...
            return $model->join('shops', 'shops.brand_id', '=', 'brands.id')->select(DB::raw(
                "6371 * acos(cos(radians(". $myLat ."))
                * cos(radians(latitude)) * cos(radians(shops.longitude) - radians(". $myLon ."))
                + sin(radians(". $myLat .")) * sin(radians(shops.latitude))) AS distance"), "brands.*")
                ->groupBy("brands.id")
                ->where('shops.active','1')
                ->orderBy('distance', 'asc');
        } else {
            return $model->join('shops', 'shops.brand_id', '=', 'brands.id')
                ->groupBy("brands.id")
                ->where('shops.active','1')
                ->select("brands.*")
