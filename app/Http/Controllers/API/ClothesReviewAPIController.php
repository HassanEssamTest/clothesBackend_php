...
class ClothesReviewAPIController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $clothesReviews = $this->clothesReviewRepository;
        if($request->pagination){
            $clothesReviews = $clothesReviews->paginate(10);
        }
        else{
            $clothesReviews = $clothesReviews->get();
        }
    ...
    public function show($id)
    {
        ...
        if (!empty($this->clothesReviewRepository)) {
            $clothesReview = $this->clothesReviewRepository->with('user')->findWithoutFail($id);
        }
